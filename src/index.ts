...
async function chatResponse (abstracts: string) {
...
async function main () {
  ...
  const abstracts = await getMedicalArticles(question)
  const response = await chatResponse(abstracts)
  console.log('response', response)
}
