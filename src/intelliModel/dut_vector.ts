...
const prompt = ChatPromptTemplate.fromMessages([
  [
    ...
    `Você deve analisar a diretriz de utilizacao de cobertura de saude fornecida e extrair as seguintes metadados essenciais:
            ...
            - numero: numero da DUT: text
            - nome: titulo completo da cobertura: string
            - indicacoes: lista de sintomas ou condicao clinica do paciente específicos que justificam a cobertura do procedimento: string[]
...
async function text2Doc(fileDir: any) {
  ...
  for (let i = 4; i < 43; i++) {
    try {
      ...
      docs.push(doc)
      console.log(`document ${i} generated ...`)
      ...
      console.log(`document ${i} not generated ...`)
    }
  }
  return docs
}
...
async function saveDocuments(
  ...
  try {
    ...
    try {
      ...
      existingDocs = JSON.parse(fileContent)
    } catch (error) {
      // If the file does not exist, it will be created
    }

    // Append new documents to existing ones
    const allDocs = existingDocs.concat(docs)

    // Write the updated document list to the file
    await fs.writeFile(filePath, JSON.stringify(allDocs, null, 2), 'utf8')
  } catch (error) {
